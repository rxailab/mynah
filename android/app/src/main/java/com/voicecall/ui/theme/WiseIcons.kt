 ...
 *
 * Material's set is close but not the same shape — its house has a door, its
 * chevron is a different weight — and at the 15–22dp these are used at, that
 * difference is most of what a row looks like. Every path here is copied from
 * the prototype rather than matched by eye.
 */
object Wise {

    /** Back. Android puts this top-left; the gesture does the same thing. */
    val ArrowLeft = stroked("ArrowLeft", "M19 12H5", "m11 18-6-6 6-6")

    /** Dismiss a full-screen dialog — the Android counterpart to iOS's swipe-down. */
    val Close = stroked("Close", "M6 6l12 12", "M18 6 6 18")

    /** "There is more behind this row." */
    val ChevronRight = stroked("ChevronRight", "m9 6 6 6-6 6")
    ...
    val Home = stroked("Home", "M3 11.5 12 4l9 7.5", "M5.5 10v9.5h13V10", width = 1.9f)
    ...
    val Check = stroked("Check", "M5 13l5 5L20 7", width = 2.8f)
    ...
    /** The history tab. The prototype's three tabs are house / phone / person;
     *  this app's first tab is the composer, so the phone moves up to it and
     *  the record of past calls needs a face of its own. */
    val Clock = stroked(
        "Clock",
    ...
    val ArrowUp = stroked("ArrowUp", "M12 19V5", "m5 12 7-7 7 7")
...
/**
 * Stroked paths are built white and recoloured at draw time: [androidx.compose.material3.Icon]
 * tints the whole painter, so the colour declared here never reaches the screen.
 */
private fun stroked(name: String, vararg paths: String, width: Float = 2.2f): ImageVector =
